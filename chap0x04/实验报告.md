...
结果可在[travis](https://travis-ci.org/CUCCS/linux-2019-Lyc-heng/builds/522033020?utm_source=github_status&utm_medium=notification)中查看
...
- 支持对jpeg格式图片进行图片质量压缩
        ...
        # bash Task_1.sh -f 文件或文件夹路径 -j 图片不能超过的大小 高 宽 压缩图片质量
...
- 支持对jpeg/png/svg格式图片在保持原始宽高比的前提下压缩分辨率
        ...
        # bash Task_1.sh -f 文件或文件夹路径 -cr 压缩分辨率
...
- 支持对图片批量添加自定义文本水印
        ...
        # bash Task_1.sh -f 文件或文件夹路径 -w 水印内容
...
- 支持批量重命名（统一添加文件名前缀或后缀，不影响原始文件扩展名）
        ...
        # bash Task_1.sh -f 文件或文件夹路径 -a -p 前缀文本 所要添加的图片类型
        bash Task_1.sh -f Timg/ -a -p pre jpg

        # bash Task_1.sh -f 文件或文件夹路径 -a -s 后缀文本 所要添加的图片类型
...
- 组合命令举例
        ...
        # 对所有jpg文件添加前缀，并将svg文件转为jpg
        bash Task_1.sh -f Timg/ -a -p test jpg -t

    ![](img/1-cmds1.PNG)

        # 修改文件分辨率，并添加后缀
        bash Task_1.sh -f Timg/3.jpg -cr 200x1080 -a -s end jpg
...
- 统计不同年龄区间范围（20岁以下、[20-30]、30岁以上）的球员数量、百分比
        ...
        <====================Age State====================>
        Number of athletes under 20 years of age:   9 people
        Percentage:1.22%
        Number of players aged 20 to 30: 600 people
        Percentage:81.52%
        Number of players over 30 years of age: 127 people
        Percentage:17.25%

- 统计不同场上位置的球员数量、百分比(发现有个很奇怪的单词，查询后是法语的后卫的意思，于是在统计时也把它算成了英语的后卫)

        <==========Player's on-site location information==========>
        Position:Defender       Number:237      percentage:32.20%
        Position:Forward        Number:135      percentage:18.34%
        Position:Goalie         Number:96       percentage:13.04%
        Position:Midfielder     Number:268      percentage:36.41%
...
- 统计访问来源主机TOP 100和分别对应出现的总次数
        ...
        lycheng@lycheng:~/chap0x04$ bash Task_3.sh web_log.tsv -th
...
- 统计访问来源主机TOP 100 IP和分别对应出现的总次数
        ...
        lycheng@lycheng:~/chap0x04$ bash Task_3.sh web_log.tsv -ti
...
- 统计最频繁被访问的URL TOP 100
        ...
        lycheng@lycheng:~/chap0x04$ bash Task_3.sh web_log.tsv -tu
...
- 统计不同响应状态码的出现次数和对应百分比
        ...
        lycheng@lycheng:~/chap0x04$ bash Task_3.sh web_log.tsv -cs
...
- 分别统计不同4XX状态码对应的TOP 10 URL和对应出现的总次数
        ...
        lycheng@lycheng:~/chap0x04$ bash Task_3.sh web_log.tsv -c4s
...
- 给定URL输出TOP 100访问来源主机
        ...
        lycheng@lycheng:~/chap0x04$ bash Task_3.sh web_log.tsv -osh /images/USA-logosmall.gif
...
- [CUCCS/2015-linux-public-songyawen](https://github.com/CUCCS/2015-linux-public-songyawen/blob/master/exp4/shell%E7%BC%96%E7%A8%8B.md#shell%E7%BC%96%E7%A8%8B)
